...
function MultiSelectionAccordian() {
  ...
  return (
    <div className="h-full w-full flex flex-col justify-center items-center ">
      ...
      <button className="mt-5" onClick={() => setEnableMultiSelection(!enableMultiSelection)}>
        ...
        {data && data.length > 0 ? (
          data.map((dataItems, index) => (
            <div
              ...
              className="border flex flex-col bg-gray-400 mt-3 rounded-xl py-1 px-3 cursor-pointer"
            >
              {selection === dataItems.id || enableMultiSelection ? (
                <div className="flex items-center justify-between gap-3">
                  <p className=" text-blue-600">Q: {dataItems.question}</p>
                  <span className="bg-red-500 h-7.5 p-2 flex items-center text-white text-xl rounded ">
                    +
                  </span>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-3">
                  <p className=" text-blue-600">Q: {dataItems.question}</p>
                  <span className="bg-gray-500 h-7.5 p-2 flex items-center text-white text-xl rounded ">
                    +
                  </span>
                </div>
              )}
